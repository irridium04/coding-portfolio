...
// arrays that hold the values in the dropdowns
...
boolean isDelivery = false; // boolean for delivery or takeout

String order; // the string that holds the entire order display

// crate a list model for toppings
...
public PizzaForm()
{
    ...
    // clears the size options dropdown
    jcmbosizes.removeAllItems();
    
    // iterate through each size in sizes array
    for (String size : sizes)
    {
        jcmbosizes.addItem(size); // add size to the sizes dropdown
    }
    
    // clears the delivery options dropdown
    jcmbodelivery.removeAllItems();
    
    // iterate through each pickup option in pickup option array
    for (String option : pickupOptions)
    {
        jcmbodelivery.addItem(option); // add option to the pickup options dropdown
    }
    
    // add toppings into topping list
...
@SuppressWarnings("unchecked")
    ...
    // call the refresh order function when actions are performed on UI elements
    
    private void jcmbosizesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jcmbosizesActionPerformed
        // TODO add your handling code here:
        RefreshOrder();
    }//GEN-LAST:event_jcmbosizesActionPerformed
    ...
    private void jcmbodeliveryActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jcmbodeliveryActionPerformed
        // TODO add your handling code here:
        RefreshOrder();
    }//GEN-LAST:event_jcmbodeliveryActionPerformed

    private void jlistToppingsValueChanged(javax.swing.event.ListSelectionEvent evt) {//GEN-FIRST:event_jlistToppingsValueChanged
        // TODO add your handling code here:
        RefreshOrder();
    }//GEN-LAST:event_jlistToppingsValueChanged
    ...
    try 
    {
        jTxtTicket.print(); // open the printer menu for the order ticket
    } 
    catch (PrinterException ex) 
    {
        // log an error if something fails with printing
    ...
    void RefreshOrder() // called on ui update
    {
        ...
        String Fname = jtxtFname.getText(); // gets order first name
        String Lname = jtxtLname.getText(); // gets order last name
        
        // arrays to store price info
        ...
        // check the index is not negative
        if(sizeIndex < 0)
        {
            sizeIndex = 0;
        }
        ...
        // makes lists based on what items are selected for toppings and extras
        // since multiple list items can be selcted at once
        ...
        switch(sizeIndex) // adding price based on pizza size
        {
            case 0: // small pizza
                price += 3;
                break;
        ...
        if(deliveryIndex == 1) // setting delivery fee if delivery is selected
        {
            price += 5;
        }
        ...
        {
            order += "\t" + topping + "\t\t\t\t 1.00\n"; // add a line on ticket
            price += 1;  // add $1 to price
        }
        
        for (String extra : extrasList) // add extras
        {
            order += "\t" + extra + "\t\t\t\t  .50\n"; // add a line on ticket
            price += .5; // add $.50 to price
        }
        
        String priceFormatted = String.format("%.2f", price); // format price rounded to 2 decimals
        order += "\n\tTotal \t\t\t\t$" + priceFormatted; // total price formatted
